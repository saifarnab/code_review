...
class _RemoveCodeModel:
    def __init__(self):
        self.df = None
        self.each_set_len = 0
        self.remove_code_dataframe = []
    ...
    def _remove_code_model(self):
        for i in range(0, self.fold):

            messages = self.df['message'].values
            remove_codes = self.df['remove_code'].values
            ...
            if (len(self.df) - (i + 1) * self.each_set_len) < self.each_set_len:
                ...
                train_y = np.delete(remove_codes, list(range(i * self.each_set_len, len(self.df))))
                test_x = messages[i * self.each_set_len: len(self.df)]
                test_y = remove_codes[i * self.each_set_len: len(self.df)]
                # print(
                ...
                train_y = np.delete(remove_codes, list(range(i * self.each_set_len, (i + 1) * self.each_set_len)))
                test_x = messages[i * self.each_set_len: (i + 1) * self.each_set_len]
                test_y = remove_codes[i * self.each_set_len:(i + 1) * self.each_set_len]
                # print(
            ...
            for idx, item in enumerate(test_x):
                ...
                self.remove_code_dataframe.append([item, test_y[idx], model_probability])

    def process(self, df):
        self.df = df
        self.each_set_len = int(len(df) / self.fold)
        self._remove_code_model()
        return pd.DataFrame(self.remove_code_dataframe, columns=['message', 'remove_code', 'prediction'])


remove_code_model = _RemoveCodeModel()
